"""
modules/telegram/smm.py — BulkFollows (SMM panel) client.
...
panel hiccup must not take down a post that already went out.
"""
...
def place_order(link: str, quantity: int, service: str) -> dict | None:
    ...
    if not config.BULKFOLLOWS_API_KEY:
        log.warning("BulkFollows: no BULKFOLLOWS_API_KEY set — order skipped (%s)", link)
        return None
    if not service:
    ...
    body = {
        "key": config.BULKFOLLOWS_API_KEY,
        "action": "add",
        "service": service,
        "link": link,
        "quantity": quantity,
    ...
    log.info("BulkFollows -> POST %s  %s", config.BULKFOLLOWS_API_URL,
             {**body, "key": _mask(body["key"])})

    try:
        # `data=` (not `json=`) is what makes this x-www-form-urlencoded.
        resp = requests.post(config.BULKFOLLOWS_API_URL, data=body, timeout=TIMEOUT)
