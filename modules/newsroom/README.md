...
Lives on the **`client/wp-newsbot`** branch, which is cut from `master` and
**never merges back**. It is a separate product that happens to share an
ancestor.
...
## The rest of this repository is inherited, not dead

Nothing outside `modules/newsroom/` is modified on this branch, and nothing is
deleted. These are fully functional and simply **not started**:
