...
| `shared/monitoring/` | Alerting, all email, all inert until `ALERT_SMTP_HOST`+`ALERT_EMAIL_TO` are set: `errmail.py` (root-logger handler — every logged ERROR/traceback is mailed per occurrence from a worker thread; `install(tag)` is called by news_bot, collector, dispatcher, telegram_bot, server.py), `heartbeat.py` (healthchecks.io dead-man pings **from inside each process's own loop** — a wedged loop must stop them; `HEALTHCHECK_URL_*` per process), `checks.py` (5-min systemd-timer CLI: CPU + hourly BulkFollows/OpenRouter balances vs `ALERT_*` floors; alert on crossing + daily reminder + recovery notice, state in `data/monitor_state.json`; a floor of 0 disables a leg — both checkouts share the VPS, so machine/account legs run only in master's timer), `mailer.py` (SMTP; never raises, never logs ≥ERROR — that would loop through errmail). See `docs/DEPLOY.md` "Monitoring". |
