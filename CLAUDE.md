...
```
modules/
  instagram/   upload_post.py (phone/ADB), graph.py (Graph API — no phone), main.py, dump_ui.py, human_swipe.py, analyze_swipes.py, visualize_swipes.py
...
| `modules/telegram/news_bot.py` | Manual broadcaster: post text/photo/video/album in the control group → inline picker of `TG_DESTINATIONS` channels **plus `YT_DESTINATIONS` YouTube channels when the post has a video, plus `TW_DESTINATIONS` Twitter accounts when the post is a single photo/video (albums excluded — X's multi-image upload path isn't built)** → caption translated per destination → Telegram fan-out and/or Shorts upload and/or tweets. Also accepts **IG reel / Twitter-X URLs**: downloads via `shared/reel_downloader` (video-first, photo fallback), caption = your text around the URL if any, else the link's own caption (the picker shows which); **replying to an open picker replaces the caption** (any post type). Telegram channel fan-out has **no size limit** (the file_id is re-sent, the bytes never leave Telegram); branding and the YouTube leg need the file on disk and go through `mtproto.py` past 20 MB, falling back to the Bot API's `get_file` with a clear error when that client isn't logged in. Downloaded files (`tg_data/media/manual_url_*`) are deleted on post/cancel and swept at startup. Single-video posts with `BRANDS` configured gate through "Post as-is / Brand it"; brand-it renders per-brand variants and publishes only explicitly selected brand→platform pairs. **Photo posts** (single photo, all-photo album, or photo URL post) with `BRANDS` gate through "Post as-is / 🖼 Create post": the first photo is the hero, the next two become the circular insets, the caption is the headline (translated per brand), and one card per selected brand is rendered via `shared/photo_card.py`, sent back as a preview, then published through the same publish picker (TG + X + IG pairs — YouTube pairs are hidden for photo renders via `render["kind"] == "photo"`). **Instagram pairs** (`BRAND_<NAME>_IG`, platform key `ig`) go through the Graph API: `_do_publish` exposes the render via `shared/public_media.expose`, calls `graph.publish_reel` (video render) or `graph.publish_photo` (card) with the brand's translated headline as caption, and takes the file down in a `finally`; result lines use the same ✅/❌ format. A daily JobQueue job (`ig-token-refresh`, first run 2 min after start) refreshes every `IG_GRAPH_ACCOUNTS` token older than 7 days and logs failures at ERROR (→ errmail). Not in the as-is picker or the autopilot yet. **Also hosts the Telegram autopilot** (`autopilot.schedule`) and the `/queue`, `/autopilot`, `/asks`, `/next` commands — one bot token allows exactly one poller, and the reaction ask needs callbacks. |
